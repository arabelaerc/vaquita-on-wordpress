...
class Ftc_Metaboxes {
    ...
    function add_meta_boxes() {
        $datas = array(
            array(
                'id' => 'page_options'
                , 'label' => esc_html__('Page Options', 'weddingshop')
                , 'post_type' => 'page'
            )
            , array(
                'id' => 'testimonial_options'
                , 'label' => esc_html__('Testimonial Options', 'weddingshop')
                , 'post_type' => 'ftc_testimonial'
            )
            , array(
                'id' => 'team_options'
                , 'label' => esc_html__('Member Information', 'weddingshop')
                , 'post_type' => 'ftc_team'
            )
            , array(
                'id' => 'portfolio_options'
                , 'label' => esc_html__('Portfolio Options', 'weddingshop')
                , 'post_type' => 'ftc_portfolio'
            )
            , array(
                'id' => 'portfolio_gallery'
                , 'label' => esc_html__('Portfolio Gallery', 'weddingshop')
                , 'post_type' => 'ftc_portfolio'
                , 'context' => 'side'
                , 'priority' => 'low'
            )
            , array(
                'id' => 'logo_options'
                , 'label' => esc_html__('Logo Options', 'weddingshop')
                , 'post_type' => 'ftc_brand'
            )
            , array(
                'id' => 'product_options'
                , 'label' => esc_html__('Product Options', 'weddingshop')
                , 'post_type' => 'product'
            )
            , array(
                'id' => 'post_options'
                , 'label' => esc_html__('Post Options', 'weddingshop')
                , 'post_type' => 'post'
            )
            , array(
                'id' => 'post_gallery'
                , 'label' => esc_html__('Post Gallery', 'weddingshop')
                , 'post_type' => 'post'
                , 'context' => 'side'
                , 'priority' => 'low'
            )
        ...
        foreach ($options as $option) {
            ...
            switch ($option['type']) {
                ...
                case 'upload':
                    ...
                    $html .= '<input type="button" class="ftc_meta_box_upload_button" value="' . esc_html__('Select Image', 'weddingshop') . '" />';
                    $html .= '<input type="button" class="ftc_meta_box_clear_image_button" value="' . esc_html__('Clear Image', 'weddingshop') . '" ' . ($post_meta_value ? '' : 'disabled') . ' />';
                    if (strlen($option['desc']) > 0) {
                        $html .= '<p class="description">' . $option['desc'] . '</p>';
                    }
                    if ($post_meta_value) {
                ...
                case 'gallery':
                    ...
                    $html .= '<a href="#" class="add-image">' . esc_html__('Add Images', 'weddingshop') . '</a>';
                    $html .= '</div>';
                    break;
