...
if( !empty($smof_data['ftc_enable_quickshop']) && ftc_has_woocommerce() && !class_exists('Ftc_Quickshop') && !wp_is_mobile() ){
	...
	class Ftc_Quickshop{
		...
		function add_quickshop_button(){
			...
			echo '<a class="quickshop" href="'.esc_url($href).'"><i class="fa fa-eye"></i><span class="ftc-tooltip button-tooltip">'.esc_html__('Quick view', 'weddingshop').'</span></a>';
		...
		function add_hook(){
			...
			/*add_action('woocommerce_after_add_to_cart_button', array($this, 'add_quickshop_button'),61 );	*/
