...
add_action('woocommerce_after_shop_loop_item', 'ftc_template_loop_short_description_listview',61); 
	...
	if ( $product->managing_stock() ) {
		if ( $product->is_in_stock() && $product->get_stock_quantity() > get_option( 'woocommerce_notify_no_stock_amount' ) ) {
			switch ( get_option( 'woocommerce_stock_format' ) ) {
				case 'no_amount' :
					$availability = esc_html__( 'In stock', 'weddingshop' );
				break;
				case 'low_amount' :
					if ( $product->get_stock_quantity() <= get_option( 'woocommerce_notify_low_stock_amount' ) ) {
						$availability = sprintf( esc_html__( 'Only %s left in stock', 'weddingshop' ), $product->get_stock_quantity() );
						if ( $product->backorders_allowed() && $product->backorders_require_notification() ) {
							$availability .= ' ' . esc_html__( '(can be backordered)', 'weddingshop' );
						}
					} else {
						$availability = esc_html__( 'In stock', 'weddingshop' );
					}
				break;
				default :
					$availability = sprintf( esc_html__( '%s in stock', 'weddingshop' ), $product->get_stock_quantity() );
					if ( $product->backorders_allowed() && $product->backorders_require_notification() ) {
						$availability .= ' ' . esc_html__( '(can be backordered)', 'weddingshop' );
					}
				break;
			...
			$availability = esc_html__( 'Available on backorder', 'weddingshop' );
			$class        = 'available-on-backorder';
		} elseif ( $product->backorders_allowed() ) {
			$availability = esc_html__( 'In stock', 'weddingshop' );
			$class        = 'in-stock';
		} else {
			$availability = esc_html__( 'Out of stock', 'weddingshop' );
			$class        = 'out-of-stock';
		}
...
function ftc_template_loop_short_description(){
	...
	<?php
		...
		if( $show_grid_desc ):
		?>
			<div class="meta-short-des grid" style="<?php echo ($has_grid_list)?'display: none':''; ?>" >
				<?php ftc_the_excerpt_max_words($grid_limit_words, '', true, '', true); ?>
			</div>
		<?php
		endif;
...
function ftc_template_loop_categories(){
	global $product;
	$categories_label = esc_html__('Categories: ', 'weddingshop');
	echo $product->get_categories(', ', '<div class="product-categories"><span>'.$categories_label.'</span>', '</div>');
}
...
function ftc_shop_top_content_widget_area_button(){
	...
	?>
		<div class="prod-cat-show-top-content-button"><a href="#"><?php esc_html_e('Filter', 'weddingshop') ?></a></div>
	<?php
	}
}
...
/* Add hook */
add_action( 'ftc_output_related_products', 'woocommerce_output_related_products', 20 );
	...
	<div class="social-sharing">
                ...
                <div class="email">
					<a href="mailto:?subject=<?php echo esc_attr(sanitize_title(get_the_title())); ?>&amp;body=<?php echo esc_url(get_permalink()); ?>">
						...
						<?php esc_html_e('Email to a Friend', 'weddingshop') ?>
					</a>
				</div>
				<div class="print">
					<a href="javascript:window.print()" rel="nofollow"><i class="fa fa-print"></i><?php esc_html_e('Print', 'weddingshop') ?></a>
				</div>
	<?php
}
...
function ftc_template_single_meta(){
	global $product, $post, $smof_data;
		...
		if ($smof_data['ftc_prod_cat']) {
			echo wc_get_product_category_list($product->get_id(),', ', '<div class="caftc-link"><span>' . esc_html__('Categories', 'weddingshop') . '</span><span class="cat-links">', '</span></div>');
		}
		if ($smof_data['ftc_prod_tag']) {
			echo wc_get_product_tag_list($product->get_id(),', ', '<div class="tags-link"><span>' . esc_html__('Tags', 'weddingshop') . '</span><span class="tag-links">', '</span></div>');
		}
...
function ftc_template_single_sku(){
	...
	if ( wc_product_sku_enabled() && ( $product->get_sku() || $product->is_type( 'variable' ) ) ){
		echo '<div class="sku-wrapper product_meta">' . esc_html__( 'Sku: ', 'weddingshop' ) . '<span class="sku" itemprop="sku">' . (( $sku = $product->get_sku() ) ? $sku : esc_html__( 'N/A', 'weddingshop' )) . '</span></div>';
	}
}
function ftc_template_single_availability(){
	...
	$product_stock = ftc_product_get_availability();
	$availability_text = empty($product_stock['availability'])?esc_html__('In Stock', 'weddingshop'):esc_attr($product_stock['availability']);
	?>	
...
function ftc_product_group_button_start(){
	...
	echo "<div class=\"button-container\" ><div class=\"group-button-image\" >";
}
...
if( class_exists('YITH_WCWL') ){
	function ftc_add_wishlist_button_to_product_list(){
		...
		echo '<div class="yith-wcwl-add-to-wishlist add-to-wishlist-'.$product->get_id().' '.$added_class.'">';
			echo '<a href="' . esc_url( add_query_arg( 'add_to_wishlist', $product->get_id() ) )
				...
				. '" class="add_to_wishlist wishlist" ><i class="fa fa-heart"></i><span class="ftc-tooltip button-tooltip">'.esc_html__('Wishlist', 'weddingshop').'</span></a>';
			...
			echo '<span class="yith-wcwl-wishlistaddedbrowse hide" style="display: none">';
				echo '<a href="'.esc_url($wishlist_url).'"><i class="fa fa-heart-o"></i><span class="ftc-tooltip button-tooltip">'.esc_html__('Added to wishlist', 'weddingshop').'</span></a>';
			echo '</span>';
	...
	/*add_action( 'woocommerce_after_add_to_cart_button', 'ftc_add_wishlist_button_to_product_list', 70 );*/
...
if( class_exists('YITH_Woocompare') && get_option('yith_woocompare_compare_button_in_products_list') == 'yes' ){
	...
	if( $yith_woocompare->is_frontend() || $is_ajax ) {
		...
		/*add_action('woocommerce_after_add_to_cart_button', 'ftc_add_compare_button_to_product_list', 70 );*/
		...
		function ftc_compare_button_text_filter( $button_text ){
			return '<i class="fa fa-refresh"></i><span class="ftc-tooltip button-tooltip">'.esc_html($button_text).'</span>';
		}
	}
}
...
function ftc_cart_continue_shopping_button(){
	echo '<a href="'.esc_url(wc_get_page_permalink('shop')).'" class="button button-secondary">'.esc_html__('Continue Shopping', 'weddingshop').'</a>';
}
?>