...
function ftc_template_redirect(){
	...
	if( is_page() || is_tax( get_object_taxonomies( 'product' ) ) || is_post_type_archive('product') ){
		...
		if( trim($ftc_page_datas['ftc_bg_breadcrumbs']) != '' ){
			$smof_data['ftc_bg_breadcrumbs'] = $ftc_page_datas['ftc_bg_breadcrumbs'];
		}
		
		if( trim($ftc_page_datas['ftc_logo']) != '' ){
			$smof_data['ftc_logo'] = $ftc_page_datas['ftc_logo'];
		}
			...
			if( !empty($term->term_id) ){
				...
				if( $bg_breadcrumbs_id != '' ){
					...
					if( $bg_breadcrumbs_src !== false ){
						$smof_data['ftc_bg_breadcrumbs'] = $bg_breadcrumbs_src;
					}
				}
				if( $layout != '' ){
	...
	if( is_singular('post') ){
		...
		if( !empty($bg_breadcrumbs) ){
			$smof_data['ftc_bg_breadcrumbs'] = $bg_breadcrumbs;
		}
	}
...
function ftc_grid_list_desc_style(){
	$custom_css = ".products.list .meta-short-des.list{display: inline-block !important;}";
	$custom_css .= ".products.grid .meta-short-des.grid{display: inline-block !important;}";
