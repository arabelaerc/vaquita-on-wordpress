...
jQuery(document).ready(function($){
	...
	$('#expand_options').click(function(){
		if(flip == 0){
			...
			$('#of_container #content').width(98+"%");
			$('#of_container .group').add('#of_container .group h2').show();
	
			$(this).removeClass('dashicons-menu expand');
			$(this).addClass('dashicons-exerpt-view close');
			$(this).text('');
					
		} else {
			flip = 0;
			$('#of_container #of-nav').show();
			$('#of_container #content').width(81+"%");
			...
			$(this).removeClass('dashicons-exerpt-view close');
			$(this).addClass('dashicons-menu expand');
			$(this).text('');
	...
	$.fn.center = function () {
		...
		this.css("left", ($(window).width() - this.width() - 160)/2  + "px");
		return this;
	}
	...
	function stickyInfo() {
        var stickyWidth = jQuery('#header').width() - 57;
		
        if (!jQuery('#info_bar').isOnScreen() && !jQuery('#ftc-footer-theme-options-sticky').isOnScreen()) {
            jQuery('#ftc-footer-theme-options').css({
                position: 'fixed'
                ,bottom: '0'
                ,width: stickyWidth
            ...
            jQuery('#ftc-footer-theme-options').addClass('sticky-footer-fixed');
        } else {
            jQuery('#ftc-footer-theme-options').css({
                background: ''
                ,position: 'inherit'
            ...
            jQuery('#ftc-footer-theme-options').removeClass('sticky-footer-fixed');
        }
    }

    if (jQuery('#ftc-footer-theme-options').length !== 0) {
	...
	var current_color_scheme = $('input[name="ftc_color_scheme"]').siblings('img.of-radio-img-selected').siblings('input[name="ftc_color_scheme"]').val();
	$('input[name="ftc_color_scheme"]').siblings('img').bind('click', function(){
		var input = $(this).siblings('input[name="ftc_color_scheme"]');
		...
		if( ok ){
			...
			$('input[name="ftc_color_scheme"][value="'+current_color_scheme+'"]').siblings('img').trigger('click');
		}
	});
