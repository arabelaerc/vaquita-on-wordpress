...
class Options_Machine {
			...
			switch ( $value['type'] ) {
				...
				case 'css_field':
					...
					if(isset($value['options'])){
						...
						$ta_value = stripslashes($smof_data[$value['id']]);			
						$output .= '<textarea class="of-input" name="'.$value['id'].'" cols="'. $cols .'" rows="8">'.$ta_value.'</textarea>';		
						$output .= '<div id="'. $value['id'] .'"></div>';
				break;
				...
				case 'backup':
					...
					if(!isset($backup['backup_log'])) {
						$log = esc_html__('No backups yet', 'weddingshop');
					} else {
					...
					$output .= '<p><strong>'. esc_html__('Last Backup : ', 'weddingshop').'<span class="backup-log">'.$log.'</span></strong></p></div>'."\n";
					$output .= '<a href="#" id="of_backup_button" class="button" title="'.esc_html__('Backup Options', 'weddingshop').'">'.esc_html__('Backup Options', 'weddingshop').'</a>';
					$output .= '<a href="#" id="of_restore_button" class="button" title="'.esc_html__('Restore Options', 'weddingshop').'">'.esc_html__('Restore Options', 'weddingshop').'</a>';
					$output .= '</div>';
				...
				case 'transfer':
					...
					$output .= '<a href="#" id="of_import_button" class="button" title="'.esc_html__('Restore Options', 'weddingshop').'">'.esc_html__('Import Options', 'weddingshop').'</a>';
				...
				case 'switch':
					...
					//Label OFF
					if(!isset($value['off'])){
						$off = esc_html__("Off", "weddingshop");
					}else{
						$off = $value['off'];
					}
				...
				case "button":
					$button_text = $value['std'];
					$output .= '<a href="javascript: void(0)" class="ftc-button">'.$button_text.'</a>';
				break;
	...
	public static function optionsframework_media_uploader_function($id,$std,$mod){
		...
		if ( function_exists( 'wp_enqueue_media' ) ) {
			$uploader .= '<span class="button media_upload_button" id="'.$id.'">'.esc_html__('Upload', 'weddingshop').'</span>';
			
			if(!empty($upload)) {$hide = '';} else { $hide = 'hide';}
			$uploader .= '<span class="button remove-image '. $hide.'" id="reset_'. $id .'" title="' . $id . '">'.esc_html__('Remove', 'weddingshop').'</span>';
		}
		else 
		{
