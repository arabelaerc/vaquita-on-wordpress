<?php
/**
 * weddingshop functions and definitions
 *
 ...
 */
/**
 * weddingshop only works in WordPress 4.7 or later.
 */
...
function ftc_custom_sidebar_form() {
    ...
    <form action="<?php echo admin_url('widgets.php'); ?>" method="post" id="ftc-form-add-sidebar">
        <input type="text" name="sidebar_name" id="sidebar_name" placeholder="<?php esc_html_e('Custom Sidebar Name', 'weddingshop') ?>" />
        <button class="button-primary" id="ftc-add-sidebar"><?php esc_html_e('Add Sidebar', 'weddingshop') ?></button>
    </form>
    <?php
}
...
$file_names = array('feedburner_subscription', 'products', 'blogs', 'blogs_tabs', 'recent_comments', 'product_categories');
foreach ($file_names as $file) {
...
function ftc_setup() {
     ...
     * If you're building a theme based on weddingshop, use a find and replace
     * to change 'weddingshop' to the name of your theme in all the template files.
     */
    load_theme_textdomain('weddingshop');
    ...
    /* Translation */
    load_theme_textdomain('weddingshop', get_template_directory() . '/languages');
    ...
    register_nav_menus(array(
        'primary' => esc_html__('Primary Navigation', 'weddingshop'),
        'vertical' => esc_html__('Vertical Navigation', 'weddingshop'),
    ));
...
function ftc_content_width() {
    ...
    /**
     * Filter weddingshop content width of the theme.
     *
     * @since weddingshop 1.0
     *
...
function ftc_fonts_url() {
     ...
     */
    $roboto = esc_html__( 'on', 'weddingshop');

    if ('off' !== $roboto) {
        $font_families = array();

        $font_families[] = 'Roboto Condensed:400,400i,700';
        $font_families[] = 'PT Sans:400,400i,700';
...
function ftc_excerpt_more($link) {
    ...
    $link = sprintf('<p class="link-more"><a href="%1$s" class="more-link">%2$s</a></p>', esc_url(get_permalink(get_the_ID())),
            /* translators: %s: Name of current post */ sprintf(esc_html__('Continue reading<span class="screen-reader-text"> "%s"</span>', 'weddingshop'), get_the_title(get_the_ID()))
    );
    return ' &hellip; ' . $link;
}
...
function ftc_colors_css_wrap() {
    ...
    <?php echo ftc_custom_colors_css(); ?>
    </style>
<?php
}
...
function ftc_scripts() {
    ...
    if ('dark' === get_theme_mod('colorscheme', 'light') || is_customize_preview() ) {
        wp_register_style( 'ftc-colors-dark',  get_template_directory_uri(). '/assets/css/colors-dark.css' , array( 'ftc-style' ), '1.0' );
                wp_enqueue_style( 'ftc-colors-dark' );
    }
    wp_enqueue_style( 'ftc-fonts', ftc_fonts_url(), array(), null );

    // Load the Internet Explorer 9 specific stylesheet, to fix display issues in the Customizer.
    if (is_customize_preview()) {
        wp_register_style( 'ftc-ie9', get_template_directory_uri() . '/assets/css/ie9.css', array( 'ftc-style' ), '1.0' );
        wp_enqueue_style( 'ftc-ie9' );
    }
    ...
    wp_register_script( 'owl-carousel', get_template_directory_uri() . '/assets/js/owl.carousel.min.js', array(), null, true );
    wp_enqueue_script( 'owl-carousel' );

    // Load the plugins-ext.
   wp_enqueue_script( 'ext', get_template_directory_uri() . '/assets/js/plugins-ext.js' , array(), null, true );
    ...
    wp_enqueue_script( 'ftc-skip-link-focus-fix', get_template_directory_uri() . '/assets/js/skip-link-focus-fix.js' , array(), '1.0', true );
    ...
    if (has_nav_menu('top')) {
        ...
        $ftc_l10n['expand'] = esc_html__('Expand child menu', 'weddingshop');
        $ftc_l10n['collapse'] = esc_html__('Collapse child menu', 'weddingshop');
        $ftc_l10n['icon'] = ftc_get_svg(array('icon' => 'angle-down', 'fallback' => true));
    }

    wp_enqueue_script( 'ftc-global', get_template_directory_uri() . '/assets/js/global.js' , array( 'jquery' ), '1.0', true );
...
function ftc_register_custom_css() {
    ...
    if (file_exists($filename_dir)) {
        wp_enqueue_style('ftc-dynamic-css', $filename);
    } else {
        ob_start();
...
if (!function_exists('ftc_theme_favicon')) {
    ...
    function ftc_theme_favicon() {
        ...
        global $smof_data;
        $favicon = isset($smof_data['ftc_favicon']) ? esc_url($smof_data['ftc_favicon']) : '';
        if (strlen($favicon) > 0):
            ?>
...
if (!function_exists('ftc_theme_logo')) {
    ...
    function ftc_theme_logo() {
        global $smof_data;
        $logo_image = isset($smof_data['ftc_logo']) ? esc_url($smof_data['ftc_logo']) : '';
...
if (!function_exists('ftc_get_search_form_by_category')) {
    ...
    function ftc_get_search_form_by_category() {
        ...
        $options = '<option value="">' . esc_html__('All categories', 'weddingshop') . '</option>';
        $options .= ftc_search_by_category_get_option_html($taxonomy, 0, 0);

        $rand = rand(0, 1000);
        $form = '<div class="ftc-search-by-category">
...
if (!function_exists('ftc_ajax_search')) {
    ...
    function ftc_ajax_search() {
        ...
        if ($results->have_posts()) {
            ...
            if (isset($results->post_count, $results->found_posts) && $results->found_posts > $results->post_count) {
                $view_all_text = sprintf(esc_html__('View all %d results', 'weddingshop'), $results->found_posts);
...
if (!function_exists('ftc_breadcrumbs')) {
    ...
    function ftc_breadcrumbs() {
        ...
        if (!empty($front_id)) {
            ...
            $home = esc_html__('Home', 'weddingshop');
        }
        $ar_title = array(
            'search' => esc_html__('Search results for ', 'weddingshop')
            , '404' => esc_html__('Error 404', 'weddingshop')
            , 'tagged' => esc_html__('Tagged ', 'weddingshop')
            , 'author' => esc_html__('Articles posted by ', 'weddingshop')
            , 'page' => esc_html__('Page', 'weddingshop')
            , 'portfolio' => esc_html__('Portfolio', 'weddingshop')
        );
...
function ftc_breadcrumbs_title($show_breadcrumb = false, $show_page_title = false, $page_title = '', $extra_class_title = '') {
    ...
    if ($show_breadcrumb || $show_page_title) {
        ...
        if ($smof_data['ftc_enable_breadcrumb_background_image']) {
            $breadcrumb_bg = esc_url($smof_data['ftc_bg_breadcrumbs']);
        }
        ...
        echo '<div class="breadcrumb-title-wrapper" ' . $style . '><div class="breadcrumb-content" ' . $style . '><div class="breadcrumb-title">';
        if ($show_page_title) {
            echo '<h1 class="heading-title page-title entry-title ' . $extra_class_title . '">' . $page_title . '</h1>';
        }
        if ($show_breadcrumb) {
            echo'</div></div>';
            echo'<div class="bread-cums container">
                    <div class= "vc-bread-cums">
                    <div class= "row">
                            <div class=" vc_col-xs-12 breadcum">';
            ftc_breadcrumbs();
            echo'</div>';
            echo'<div class=" vc_col-xs-12 before-loop-wrapper">';
...
function ftc_register_google_font($iframe = false) {
    ...
    if ($smof_data['ftc_body_font_enable_google_font']) {
        $fonts[] = array(
            'name' => $smof_data['ftc_body_font_google']
    ...
    if ($smof_data['ftc_secondary_body_font_enable_google_font']) {
        $fonts[] = array(
            'name' => $smof_data['ftc_secondary_body_font_google']
            , 'bold' => '300,400,500,600,700,800,900'
        );
    }
...
function ftc_register_required_plugins() {
    ...
    $plugins = array(
        array(
            ...
            'source'             => get_template_directory() . '/inc/plugins/' . 'themeftc.zip', // The plugin source.
        ...
        , array(
            ...
            'source' => get_template_directory() . '/inc/plugins/' . 'mega_main_menu.zip', // The plugin source.
            'required' => false, // If false, the plugin is only 'recommended' instead of required.
        ...
        , array(
            'name' => 'Widget Data - Setting Import/Export Plugin', // The plugin name.
