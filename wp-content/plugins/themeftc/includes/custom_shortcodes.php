...
function ftc_banner_shortcode($atts, $content = '') {
    ...
    ob_start();
    ?>
    <div class="ftc-banner <?php echo esc_attr($unique_class) ?> <?php echo esc_attr($style_effect) ?> <?php echo ($opacity_bg_device) ? 'opacity-bg-device' : '' ?> <?php echo ($responsive_size) ? 'responsive-size' : '' ?> <?php echo esc_attr($position_content) ?> <?php echo esc_attr($extra_class) ?>">
        <?php echo trim($style); ?>
...
if (!function_exists('ftc_single_image_shortcode')) {
    ...
    function ftc_single_image_shortcode($atts) {
        ...
        ?>
        <?php if ($link != ''): ?>
            <a class="ftc-smooth-image <?php echo esc_attr($style_effect) ?>" title="<?php echo esc_attr($link_title) ?>" target="<?php echo esc_attr($target); ?>" href="<?php echo esc_url($link) ?>" >
        ...
        <?php else: ?>
            <div class="ftc-smooth-image <?php echo esc_attr($style_effect) ?>">
                <div class="ftc-smooth">
                    <?php
                    if ($img_url != '') {
                        ?>
...
if (!function_exists('ftc_brands_slider_shortcode')) {
    ...
    function ftc_brands_slider_shortcode($atts, $content = null) {
        ...
        if ($brands->have_posts()):
            ...
            ?>
            <div class="ftc-brand-slider-block ftc-shortcode <?php echo esc_attr(implode(' ', $classes)); ?>" <?php echo implode(' ', $data_attr); ?>>
                <?php if (strlen($title) > 0): ?>
...
if (!function_exists('ftc_feedburner_subscription_shortcode')) {
    ...
    function ftc_feedburner_subscription_shortcode($atts) {
        ...
        echo '<div class="ftc-feedburner-subscription-shortcode ' . $style . '">';
...
if (!function_exists('ftc_blogs_shortcode')) {
    ...
    function ftc_blogs_shortcode($atts, $content = null) {
        ...
        if ($posts->have_posts()):
            ...
            $classes = array();
            $classes[] = 'ftc-sb-blogs ftc-shortcode ftc-blogs';
            if ($is_slider) {
                    ...
                    <div class="blog-ftc-wrp">
                        ...
                        <?php if ($post_format != 'quote'): ?>
                            ...
                            <div class="post-info">
                                ...
                                <?php if ($show_date && $show_thumbnail && ( $post_format == 'gallery' || $post_format === false || $post_format == 'standard' )): ?>
                                    <div class="date-time">
                                        <span><i class="fa fa-calendar"></i>
                                            <?php echo get_the_time('M'); ?> <?php echo get_the_time('d'); ?> , <?php echo get_the_time('Y'); ?>
                                        </span>
                                    </div>
                                <?php endif; ?>
                                ...
                                <!-- Blog Author -->
                                <?php if ($show_author): ?>
                                    <span class="author"><i class="fa fa-user"></i><?php esc_html_e('Admin') ?></span>
                                <?php endif; ?>
                                ...
                                <?php if ($show_excerpt && function_exists('ftc_the_excerpt_max_words')): ?>
                                    <div class="excerpt"><?php ftc_the_excerpt_max_words($excerpt_words, '', true, '', true); ?></div>
                                <?php endif; ?>
...
if (!function_exists('ftc_milestone_shortcode')) {
    ...
    function ftc_milestone_shortcode($atts) {
        ...
        ob_start();
        ?>
        <div class="ftc-milestone <?php echo esc_attr($text_color_style) ?>" data-number="<?php echo esc_attr($number); ?>">
            <span class="number">
...
if (!function_exists('ftc_products_slider_shortcode')) {
    ...
    function ftc_products_slider_shortcode($atts, $content) {
        ...
        $classes = array();
        $classes[] = 'ftc-product-slider ftc-slider';

        if ($products->have_posts()):
            ?>
            <div class="<?php echo esc_attr(implode(' ', $classes)); ?>" data-slider="<?php echo esc_attr((int) $is_slider) ?>" data-columns="<?php echo esc_attr($columns) ?>"
...
if (!function_exists('ftc_product_deals_slider_shortcode')) {
    ...
    function ftc_product_deals_slider_shortcode($atts, $content = null) {
        ...
        if ($products->have_posts()):
            ...
            ?>
            <div class="ftc-product-deals-slider-wrapper ftc-slider ftc-shortcode ftc-product <?php echo esc_attr(implode(' ', $classes)); ?>" <?php echo implode(' ', $data_attr); ?>>
                <header class="shortcode-heading-wrapper">
                    <?php if (strlen($title) > 0): ?>
                        <h3 class="heading-title"><span class="bg-heading"><span><?php echo esc_html($title); ?></span></span></h3>
                    <?php endif; ?>
